...
def main():
   try:
       ...
       # POST: single delete or bulk delete, then redirect so list is refreshed
       if method == "POST":
           form = cgi.FieldStorage()
           # for single-delete button
           single_delete = (form.getfirst("single_delete") or "").strip()
           # for bulk delete checkboxes
           selected_sections = form.getlist("selected_sections")
           # filters to preserve
           name = (form.getfirst("name") or "").strip()
           wild_card = (form.getfirst("wild_card") or "").strip()
           tag_name_filter = (form.getfirst("tag_name_filter") or "").strip()
           tag_value_filter = (form.getfirst("tag_value_filter") or "").strip()
           page = (form.getfirst("page") or "").strip()
           keep_qs = {
               "name": name,
               "wild_card": wild_card,
           ...
           try:
               ...
               if not items:
                   messages.append(("danger", "No section tags selected to delete"))
                   keep_qs["deleted"] = "0"
               else:
                   success_count = 0
                   for item in items:
                       # each value is "d2l_genius_tagEntryId"
                       ...
                       success_count += 1
                   if success_count:
                       messages.append(("success", f"{success_count} section tag(s) deleted"))
                       keep_qs["deleted"] = "1"
                   else:
                       messages.append(("danger", "Delete failed: no valid rows selected"))
                       keep_qs["deleted"] = "0"
           except Exception as e:
               messages.append(("danger", f"Delete failed: {e}"))
               keep_qs["deleted"] = "0"
       ...
       sys.stdout.write(f"<h1>section_tags.py crashed</h1><pre>{esc}</pre>")
